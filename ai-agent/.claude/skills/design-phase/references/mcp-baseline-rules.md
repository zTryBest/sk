...
- 每个最终选中的 API 必须经过 `get_api_detail`，并把 `method`、`api_path`、请求参数/契约、响应结果/契约写入 `design-handoff.json.selected_baseline_apis`。
...
将 `get_api_detail` 返回结果中的接口路径、请求参数、请求体、响应字段、错误响应和示例整理到 `design-handoff.json.selected_baseline_apis[].request` 与 `.response`。如果详情为空、字段覆盖不足或无法判断响应结果，不能把该 API 标记为最终采纳；应转为候选确认、补知识库或改定制设计。
