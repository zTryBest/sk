...
- 选中的基线 API 必须保留 `method`、`api_path`、请求参数/契约和响应结果/契约；只写 API 名称、组件名或自然语言摘要不算可交接。
- 选中的基线 API 必须写入版本兼容证据：`resolved_doc_version`、`contract_doc_version`、`version_match_policy`、`version_compatibility=PASS`。`contract_doc_version` 不能高于 `resolved_doc_version`。
...
```text
python <design-phase-skill-dir>/scripts/validate_design.py --handoff <交接目录>/design-handoff.json --output <交接目录>/design-validation.json --project-root <项目根目录>
```
...
- [ ] 每个最终 API 都写入了接口路径、请求参数/契约和响应结果/契约。
- [ ] 每个最终 API 都写入了版本兼容证据，且不存在低版本组件采纳高版本契约。
