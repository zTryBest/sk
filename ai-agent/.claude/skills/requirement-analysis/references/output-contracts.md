...
3. 先生成 `requirement-handoff.json`，作为后续 design-phase 的机器可读唯一事实来源。即使仍有关键澄清点，也要写 `source.requirement_status=draft` 和 `open_questions`。
4. 生成或运行 validator 得到 `requirement-validation.json`；校验失败不能交接 design-phase。
5. 在 worker 模式下，一旦已知道状态，就尽早写 `worker-result.json`：草稿/待确认写 `NEED_USER_INPUT`，最终版且 validator 通过写 `STAGE_COMPLETED`。
6. 关键澄清点已全部解决时写 `需求分析.md`。
7. 仍有关键澄清点时只能写 `需求分析-草稿.md`。
8. 生成 `design-phase-handoff.md`，作为后续设计阶段的人类可读入口。
9. 写入后重新读取文件，确认内容不是空文件、不是摘要占位符，并包含“功能项清单”“平台依赖和数据来源”“澄清记录”“交接给 design-phase 的上下文”。
...
```text
python <requirement-analysis-skill-dir>/scripts/validate_requirement.py --handoff <交接目录>/requirement-handoff.json --output <交接目录>/requirement-validation.json --project-root <项目根目录>
```

当前 skill 自带最小 validator 脚本；如果执行环境找不到脚本，不能假装已运行，应在 `requirement-validation.json` 或最终汇报中明确说明缺少校验脚本，并将阶段标记为不可交接或 `BLOCKED`，除非 orchestrator 提供了等价 validator。
