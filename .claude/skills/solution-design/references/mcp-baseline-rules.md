...
- 每个最终选中的 API 必须经过 `get_api_detail`，并把 `method`、`api_path`、请求参数/契约、响应结果/契约写入 `artifacts/02_solution.json.selected_baseline_apis`。
- 版本匹配是硬约束：目标组件版本解析出的 `resolved_doc_version` 只能使用小于或等于它的 `contract_doc_version`。低版本组件不能采纳高版本才出现的 API 或契约。
...
4. 对每个涉及基线 API 的组件段调用或读取 `resolve_component_doc_version(component_id, segment_id, component_version)` 的结果，记录当前组件版本可用的 `resolved_doc_version`。无法解析时不能继续采纳 API，必须转用户确认或补知识库。
5. 将 Phase 2.5 输出的 MCP 检索任务展开为队列：

| 检索任务编号 | 来源需求项 | 子能力 | 实现方式分类 | 平台上下文动作 | 依赖对象/数据来源 | 查询词 | 预期字段覆盖 |
|---|---|---|---|---|---|---|---|

6. 人工审阅模式下，检索前展示检索队列表并等待确认。
7. 对每个检索任务单独调用 `find_apis_for_requirement`：
...
- 不可用：无契约、接口删除、组件不在平台基线范围内、契约版本高于当前组件可用文档版本。
...
将 `get_api_detail` 返回结果中的接口路径、请求参数、请求体、响应字段、错误响应和示例整理到 `artifacts/02_solution.json.selected_baseline_apis[].request` 与 `.response`。如果详情为空、字段覆盖不足或无法判断响应结果，不能把该 API 标记为最终采纳；应转为候选确认、补知识库或改定制设计。

每个最终采纳 API 必须同时写入版本兼容证据：
...
如果 MCP 返回 `NO_COMPATIBLE_CONTRACT`、`NEED_KB_IMPORT`、版本不可比较，或 `contract_doc_version > resolved_doc_version`，不能采纳为最终 API。只能选择补知识库、人工确认组件/文档版本映射、改为定制实现，或等待 MCP 修正后重查。
